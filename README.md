...
3. **获取模型**（默认目录名与 `scripts/download_model.sh` 一致）
   ...
   将 `HUGGINGFACE_REPO` 改成你在 Hugging Face 上实际发布的仓库（见下文「获取模型」）。若仓库尚不存在，请先把导出的 CT2 目录**手动复制**到 `~/Library/Rime/predict_models/zh-base-ct2-int8/`，并确保含 `shared_vocabulary.json`。
...
- **推荐**：自行训练并导出为 CTranslate2 格式，上传到 [Hugging Face](https://huggingface.co)，再在本地设置环境变量后运行下载脚本：
  ...
  默认安装路径：`~/Library/Rime/predict_models/zh-base-ct2-int8/`。在方案里使用相对路径：
...
- **模型 Card 模板**：上传 HF 时可参考 [`docs/HUGGINGFACE_MODEL_CARD.md`](docs/HUGGINGFACE_MODEL_CARD.md)（训练数据、License、输入输出格式说明）。

- **手动放置**：将包含 `shared_vocabulary.json`、`tokenizer.json`、`model.bin`、`config.json` 的目录放到 `~/Library/Rime/predict_models/zh-base-ct2-int8/`（或与 `model_path` 一致的路径）。
