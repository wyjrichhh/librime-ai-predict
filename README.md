...
本仓库只是一个 librime 插件，不能单独运行。要实际"输入拼音→看到 AI 候选"，必须配合一个**会处理 `ai_predict/ready` 属性通知**的前端。我们提供了一个鼠须管 fork（`feat/ai-inference` 分支），自带必要的前端增强。
...
> 注意：仅独立编译 librime 是**不能直接体验输入法效果**的——你还需要一个能监听 `ai_predict/ready` 属性通知的前端。鼠须管原版（不打本 fork 的补丁）也能输入，但 AI 候选不会自动刷新。
