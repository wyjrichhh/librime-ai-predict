...
1. **拉取 librime 与本插件，并把插件放进 librime 的 `plugins/` 目录**
   ...
   >
   > 高级：也可以把插件放在 `plugins/` 之外，再用 `export RIME_PLUGINS="librime-ai-predict"` 让 librime 找到它。
...
2. **编译 CTranslate2（插件目录内，一次即可）**
   ...
   > 高级：若 CT2 已装在其他前缀（如系统目录），可在 step 3 编译 librime 时改用 `cmake . -Bbuild -DCTRANSLATE2_ROOT=/path/to/ct2/prefix ...` 指向它，跳过本步骤。
